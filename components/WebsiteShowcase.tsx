...
export default function WebsiteShowcase() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setCurrentIndex((prev) => (prev + 1) % websiteImages.length);
        }, 2000);
    ...
    const prev = () => {
        setCurrentIndex((p) => (p - 1 + websiteImages.length) % websiteImages.length);
    };

    const next = () => {
        setCurrentIndex((p) => (p + 1) % websiteImages.length);
    };
    ...
    const getVisibleImages = () => {
        const images = [];
        for (let i = -1; i <= 1; i++) {
            const idx = (currentIndex + i + websiteImages.length) % websiteImages.length;
            images.push({ src: websiteImages[idx], index: idx, offset: i });
        }
        return images;
    };
                    ...
                    <div className="overflow-hidden px-8 md:px-4">
                        <div className="hidden md:flex gap-6 items-center justify-center">
                            {getVisibleImages().map((img) => (
                                ...
                                >
                                    <img
                                        src={img.src}
                                        alt={`Website interface ${img.index + 1}`}
                        ...
                        <div className="md:hidden flex justify-center">
                            <div className="rounded-xl overflow-hidden border-2 border-[#FFB800]/50 shadow-2xl shadow-[#FFB800]/20 w-full">
                                <img
                                    src={websiteImages[currentIndex]}
                                    alt={`Website interface ${currentIndex + 1}`}
                                    className="w-full h-auto object-cover"
                                    loading="lazy"
                                />
                            </div>
